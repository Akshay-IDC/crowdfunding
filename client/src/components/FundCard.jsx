...
const FundCard = ({ owner, title, description, target, deadline, amountCollections, image, handleClick }) => {
  const remainingDays = daysLeft(deadline);
  
  if (amountCollections >= target) {
    return null; // Don't render the card if the target is met or exceeded
  }
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="flex justify-between flex-wrap mt-[15px] gap-2">
          <div className="flex flex-col">
            <h4 className="font-epilogue font-semibold text-[14px] text-[#b2b3bd] leading-[22px]">{amountCollections}</h4>
